...
export default function SmartAdviceCard({ response: advice, isLoading, error }: SmartAdviceCardProps) {
  const cardBaseClass = "card p-5 text-white rounded-xl shadow-lg text-gray-800 ";
  ...
  if (isLoading) {
    ...
    <div className="max-w-[100%] mt-10 p-6 bg-white rounded-2xl shadow-md flex flex-col space-y-4 mb-3">
      <div className="flex items-center space-x-3">
        <h2 className="text-2xl font-semibold text-gray-800">TechFinance AI</h2>
        <WaveIcon />
      </div>
            <div className=" max-h-[250px] overflow-y-auto">
          <h3 className={`${headerClass} animate-pulse flex items-center text-center`}>
  ...
  const response = JSON.parse(advice);
    return (
      <div className={`${cardBaseClass} bg-gradient-to-r from-finance-primary to-finance-primary-light `}>
             <div className="flex items-center space-x-3">
        <h2 className="text-2xl font-semibold text-gray-800">TechFinance AI</h2>
        <WaveIcon />
      </div>
      <div className=" max-h-[250px] overflow-y-auto">
        <span className={textClass} dangerouslySetInnerHTML={{ __html: response.choices[0].message.content }} />
